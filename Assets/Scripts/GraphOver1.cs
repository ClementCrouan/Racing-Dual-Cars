...
/// </summary> 
public class GraphOver1 : MonoBehaviour
{
	[Serializable]
	public class WheelConfig1
	{
	...
	public Rigidbody vehicleBody1;
	[Range(0f, 10f)]
	public float timeTravel;
	public List<WheelConfig1> wheelConfigs = new List<WheelConfig1>();
	...
	public void SetupWheelConfigs()
	{
		...
		if (vehicleBody1)
		{
			foreach (var wheel in vehicleBody1.GetComponentsInChildren<WheelCollider>())
			{
				var wheelConfig = new WheelConfig1();
	...
	void Update()
	{
		...
		if (vehicleBody1)
			m_SpeedText.text = string.Format("Speed: {0:0.00} m/s", vehicleBody1.velocity.magnitude);
	}
